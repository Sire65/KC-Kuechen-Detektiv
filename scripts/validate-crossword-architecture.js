...
if(exists(path.join(gameDir,'daily.js'))){
  ...
  const dailyVersion=daily.match(/const\s+GAME_VERSION\s*=\s*['"]([^'"]+)['"]/);
  if(!dailyVersion)errors.push('daily.js: GAME_VERSION fehlt');
  else if(dailyVersion[1]!==moduleJson.version)errors.push(`Versionskonflikt: daily.js ${dailyVersion[1]} vs module.json ${moduleJson.version}`);
  ...
  if(!daily.includes('completionRecorded'))errors.push('daily.js: Doppelabschluss-Sperre fehlt');
}

if(exists(path.join(gameDir,'navigation.js'))){
  const navigation=read(path.join(gameDir,'navigation.js'));
  if(!navigation.includes('scheduleRebuild'))errors.push('navigation.js: debounced rebuild fehlt');
  if(!navigation.includes('kcNavBound'))errors.push('navigation.js: Schutz vor mehrfacher Listenerbindung fehlt');
}
...
if(exists(path.join(gameDir,'motivation.js'))){
  ...
  if(!motivation.includes('breakStreak'))errors.push('motivation.js: Abbruch muss Serie zurücksetzen');
  if(!motivation.includes('roundActive'))errors.push('motivation.js: Rundenzustand für echte Serienlogik fehlt');
}
...
console.log(`Kreuzworträtsel-Architektur-TÜV OK · Modul ${moduleJson.version} · ${requiredFiles.length} Pflichtdateien · FUTURA/Manifest/Daily/Navigation/Serie geprüft.`);
