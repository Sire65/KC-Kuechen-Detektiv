(function(){
  ...
  function award(id,newOnes){if(data.achievements.includes(id))return;data.achievements.push(id);newOnes.push(id);}
  ...
  function breakStreak(){
    ...
    roundActive=false;
    if(data.streak!==0){data.streak=0;save(data);render();}
  }
  ...
  function resetForNewRound(){lastSignature='';roundActive=true;$('#resultPanel').hidden=true;}
  function newPuzzle(){lastSignature='';$('#resultPanel').hidden=true;$('#startBtn')?.click();window.scrollTo({top:0,behavior:'smooth'});}
  function choosePuzzle(){roundActive=false;lastSignature='';$('#resultPanel').hidden=true;$('#game').hidden=true;$('#start').hidden=false;window.scrollTo({top:0,behavior:'smooth'});}
  ...
  $('#solveBtn')?.addEventListener('click',breakStreak);
  $('#backBtn')?.addEventListener('click',()=>setTimeout(()=>{if($('#game')?.hidden)breakStreak();},0));
  render();
})();
