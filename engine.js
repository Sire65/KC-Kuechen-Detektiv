(function(){
  ...
  let activeScenes=[];
  const state={index:0,total:0,attempts:0,hints:0,streak:0,startedAt:0,locked:false,results:[],foundDifferences:0,foundMap:{}};
  ...
  function refreshStartInfo(){
    ...
    $('#startInfo').textContent=selected.length?`${selected.length} passende Fälle stehen bereit.`:'Für diese Auswahl gibt es noch keine Fälle.';
    $('#startBtn').disabled=!selected.length;
  }
  function beginSession(){
    activeScenes=buildSelection();if(!activeScenes.length)return;
    state.index=0;state.total=0;state.streak=0;state.results=[];
  ...
  function finish(){
    ...
    const max=activeScenes.reduce((n,s)=>s.mode==='spot_difference'?n+(s.basePoints||(DIFF[s.difficulty]||{}).base||0)+100:n+((DIFF[s.difficulty]||{}).base||0)+70,0);const pct=max?Math.round(state.total/max*100):0;
    $('#playScreen').innerHTML=`<section class="finish"><span class="eyebrow">KC FUTURA · SPIELEWELT</span><h2>${activeScenes.length} Fälle abgeschlossen</h2><p class="big">${state.total} Punkte</p><p>Leistungswert: ${pct}%</p><p>${rank(state.total,activeScenes.length)}</p><p>Bestwert: ${saved.bestScore}</p><button id="againBtn">Neue Runde</button></section>`;$('#againBtn').onclick=()=>location.reload();
  }
